...
# Target branch in the c_library repo. Defaults to master, but can be overridden
# from the environment (e.g. by CI for a workflow_dispatch run on a feature branch).
CLIBRARY_GIT_BRANCHNAME=${CLIBRARY_GIT_BRANCHNAME:-master}
...
# For a non-master target (feature/PR branch), create/reset the branch on top of
# the freshly-cloned default branch so each run is a clean regeneration snapshot.
if [ "$CLIBRARY_GIT_BRANCHNAME" != "master" ]; then
  git checkout -B "$CLIBRARY_GIT_BRANCHNAME" || exit 1
fi
...
if [ "$CLIBRARY_GIT_BRANCHNAME" = "master" ]; then
  ...
  # Force-push the regenerated snapshot to the feature branch (it is disposable
  # and rebuilt from scratch on every dispatch).
  git push --force $CLIBRARY_GIT_REMOTENAME "HEAD:$CLIBRARY_GIT_BRANCHNAME" || exit 1
fi
